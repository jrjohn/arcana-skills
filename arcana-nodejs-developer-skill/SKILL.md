...
4. **Add features by copying the nearest working entity, then adapting** (see [🔁 Adding a feature](#-adding-a-feature--copy-the-nearest-working-feature-not-from-scratch)) — the [File-by-File Feature Recipe](#file-by-file-feature-recipe--new-entity-end-to-end) below is the completeness checklist.
...
2. **Duplicate its ENTIRE file set** (domain model, Prisma schema + migration, DTO + mapper, repository interface + implementation, service interface + implementation, controller + route registration, gRPC proto + servicer, InversifyJS DI wiring, mock data, and unit tests per layer) 1:1, keeping every layer.
