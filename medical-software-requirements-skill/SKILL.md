...
description: |
  ...
  ⚠️ 重要：讀取心理學檔案時，請使用以下完整路徑：
     ~/.claude/skills/medical-software-requirements-skill/references/{filename}.md

  1️⃣ 設計心理學 (Design Psychology)
     📄 檔案：~/.claude/skills/medical-software-requirements-skill/references/design-psychology.md
  ...
  ⚠️ 重要：當 SDD 文件產出完成後，必須自動執行以下流程：

  📌 Step 1: 自動啟用 app-uiux-designer.skill
     - 讀取 SDD 中定義的所有畫面 (SCR-*)
     - 解析畫面流程與導航關係

  📌 Step 2: 產生 HTML UI Flow
     - 產出互動式 HTML 原型 (每個畫面一個 HTML 檔案)
     - 產出 index.html 導覽頁
     - 產出 ui-flow-diagram.html 互動式流程圖
     - 所有按鈕與連結皆可點擊導航

  📌 Step 3: 更新 SDD 文件
     - 將產生的 UI 畫面截圖嵌入 SDD
     - 更新 SDD 的 UI Flow 章節
     - 確保 SCR-* ID 與 HTML 檔案對應

  📌 Step 4: 同步更新 SRS 文件
     - 將 UI 畫面對應的需求 ID (REQ-*) 更新到 SRS
     - 確保 SRS 的 Screen Reference 與 SDD 的 SCR-* 對應一致
     - 更新 SRS 的 UI/UX Requirements 章節

  📌 Step 5: 同步產生 DOCX
     - 重新產生 SDD.docx 以包含最新 UI 畫面
     - 重新產生 SRS.docx 以包含最新畫面參照
---
...
## 🚀 SDD 產出後自動 HTML UI Flow 產生 (Critical)

> ### ⚠️ 強制規則：SDD 產出後必須自動產生 HTML UI Flow
>
> 當 SDD 文件產出完成後，**必須自動啟用 `app-uiux-designer.skill`** 產生互動式 HTML UI Flow，
> 並將產生的畫面更新回 SDD 文件。此流程為強制執行，不需詢問使用者。

### 自動觸發條件

以下情況會自動觸發 HTML UI Flow 產生：

| 觸發條件 | 說明 |
|----------|------|
| SDD 新建完成 | 新建立的 SDD 文件完成後 |
| SDD UI 章節更新 | SDD 的 UI/UX 設計章節有修改時 |
| SCR-* 畫面定義變更 | 畫面 ID 或流程有新增/修改時 |
...
│  Step 1: 自動啟用 app-uiux-designer.skill                       │
│  ────────────────────────────────────────────────────────────  │
│  1. 讀取 SDD 文件中的所有 SCR-* 畫面定義                          │
│  2. 解析 UI Flow (畫面之間的導航關係)                             │
│  3. 提取 Design Tokens (色彩、字型、間距)                         │
└────────────────────────────────────────────────────────────────┘
     │
     ▼
┌────────────────────────────────────────────────────────────────┐
│  Step 2: 產生 HTML UI Flow                                      │
│  ────────────────────────────────────────────────────────────  │
│  產出目錄: ./generated-ui/{ProjectName}/                        │
│                                                                 │
│  📁 generated-ui/{ProjectName}/                                 │
│  ├── 📄 index.html              # 導覽頁 (所有畫面入口)          │
│  ├── 📄 ui-flow-diagram.html    # 互動式流程圖                   │
│  ├── 📁 auth/                   # 認證模組                       │
...
│  Step 3: 擷取畫面截圖                                            │
│  ────────────────────────────────────────────────────────────  │
│  1. 將每個 HTML 畫面擷取為 PNG 圖片                               │
│  2. 存放位置: 02-design/SDD/images/ 或 screenshots/              │
│  3. 命名格式: SCR-{MODULE}-{NUMBER}-{description}.png            │
└────────────────────────────────────────────────────────────────┘
     │
     ▼
┌────────────────────────────────────────────────────────────────┐
│  Step 4: 更新 SDD 文件                                           │
│  ────────────────────────────────────────────────────────────  │
│  1. 將 UI 畫面截圖嵌入 SDD 的 UI/UX 設計章節                      │
│  2. 更新 UI Flow Diagram (Mermaid 或 圖片)                       │
│  3. 確保 SCR-* ID 與實際 HTML 檔案對應                           │
│  4. 更新 SDD.md 檔案                                             │
└────────────────────────────────────────────────────────────────┘
     │
     ▼
...
│  Step 5: 同步更新 SRS 文件                                        │
│  ────────────────────────────────────────────────────────────  │
│  1. 將 UI 畫面對應的需求 ID (REQ-*) 更新到 SRS                     │
│  2. 確保 SRS 的 Screen Reference 與 SDD 的 SCR-* 對應一致         │
│  3. 更新 SRS 的 UI/UX Requirements 章節                           │
│  4. 更新 SRS.md 檔案                                              │
└────────────────────────────────────────────────────────────────┘
     │
     ▼
...
│  ────────────────────────────────────────────────────────────  │
│  # 重新產生 SDD.docx                                              │
│  node ~/.claude/skills/medical-software-requirements-skill/    │
│       md-to-docx.js SDD-{ProjectName}-{Version}.md              │
│                                                                 │
│  # 重新產生 SRS.docx                                              │
│  node ~/.claude/skills/medical-software-requirements-skill/    │
│       md-to-docx.js SRS-{ProjectName}-{Version}.md              │
└────────────────────────────────────────────────────────────────┘
```

### SDD 更新內容

當 HTML UI Flow 產生完成後，SDD 文件會自動更新以下內容：

| SDD 章節 | 更新內容 |
|----------|----------|
| **UI/UX Design** | 嵌入各畫面 PNG 截圖 |
| **Screen Flow** | 更新 Mermaid 流程圖或 ui-flow-diagram |
| **Screen List** | 確認 SCR-* ID 與 HTML 檔案對應 |
| **Navigation** | 更新按鈕導航關係表 |

### SRS 更新內容

當 HTML UI Flow 產生完成後，SRS 文件會自動更新以下內容：

| SRS 章節 | 更新內容 |
|----------|----------|
| **UI/UX Requirements** | 確認 REQ-UI-* 與 SCR-* 對應關係 |
| **Screen Reference** | 更新各需求對應的畫面 ID |
| **User Story** | 確認 User Story 的 Acceptance Criteria 與畫面對應 |
| **Traceability Matrix** | 更新 REQ → SCR 追溯矩陣 |
...
**Screen Reference:** SCR-AUTH-001

**Acceptance Criteria:**
- [ ] 使用者可輸入 Email/Password 登入
- [ ] 支援「忘記密碼」功能連結
- [ ] 支援「註冊」功能連結

**Related Screens:**
| Screen ID | Screen Name | Relationship |
|-----------|-------------|--------------|
| SCR-AUTH-001 | Login | Primary |
| SCR-AUTH-002 | Register | Navigation |
| SCR-AUTH-003 | Forgot Password | Navigation |
```
...
![Login Screen](./images/SCR-AUTH-001-login.png)

**Screen Description:** 使用者登入畫面，支援 Email/Password 登入與社群登入。

**Navigation:**
| Button | Target | Description |
|--------|--------|-------------|
| Sign In | SCR-DASH-001 | 登入成功後導向 Dashboard |
| Forgot Password | SCR-AUTH-003 | 導向忘記密碼頁面 |
| Sign Up | SCR-AUTH-002 | 導向註冊頁面 |

**HTML File:** `generated-ui/{Project}/auth/SCR-AUTH-001-login.html`
```
...
# 1. Claude 自動啟用 app-uiux-designer.skill
#    (讀取 SDD 畫面定義，產生 HTML UI Flow)

# 2. 產生的 HTML 檔案位置
ls ./generated-ui/{ProjectName}/

# 3. 更新後的 SDD 位置
ls ./02-design/SDD/SDD-{ProjectName}-{Version}.md

# 4. 更新後的 SRS 位置
ls ./01-requirements/SRS/SRS-{ProjectName}-{Version}.md

# 5. 重新產生 DOCX (SDD + SRS)
node ~/.claude/skills/medical-software-requirements-skill/md-to-docx.js \
     ./02-design/SDD/SDD-{ProjectName}-{Version}.md

node ~/.claude/skills/medical-software-requirements-skill/md-to-docx.js \
     ./01-requirements/SRS/SRS-{ProjectName}-{Version}.md
```

### 注意事項

- ✅ 此流程為 **強制自動執行**，SDD 產出後無需使用者額外指示
- ✅ 產生的 HTML 可直接在瀏覽器開啟預覽
- ✅ 所有按鈕與連結皆可點擊導航
- ✅ ui-flow-diagram.html 支援縮放與拖曳
- ✅ **SRS 與 SDD 同步更新**，確保需求與設計的追溯一致性
- ⚠️ 若 SDD 中的 SCR-* 定義不完整，會先提示補充後再產生
...
**設計心理學:**
- `references/design-psychology.md` - **設計心理學原則指南** ⭐
  - 路徑: `~/.claude/skills/medical-software-requirements-skill/references/design-psychology.md`
...
**認知心理學:**
- `references/cognitive-psychology.md` - **認知心理學原則指南** ⭐
  - 路徑: `~/.claude/skills/medical-software-requirements-skill/references/cognitive-psychology.md`
  - 心智模型 (Mental Model)
...
**文件編排心理學:**
- `references/document-layout-psychology.md` - **技術文件編排心理學指南** ⭐ (NEW)
  - 路徑: `~/.claude/skills/medical-software-requirements-skill/references/document-layout-psychology.md`
