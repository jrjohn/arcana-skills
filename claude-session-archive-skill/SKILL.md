...
| `auto-osearch-on-prompt.sh` | UserPromptSubmit | **Proactive**. On prompts containing identity / history / status / question keywords, auto-runs `crs osearch <prompt>`, injects top hits as `additionalContext`, and pre-sets/refreshes the sentinel |
...
└── scripts/
    ...
    ├── auto-osearch-on-prompt.sh     # UserPromptSubmit hook — auto-osearch on identity/history/status prompts, injects top hits + pre-sets sentinel
