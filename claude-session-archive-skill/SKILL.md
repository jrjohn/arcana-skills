...
description: Cross-platform (macOS / Linux / Windows / **containerized / cloud-agent**) cross-session full-text + semantic history of every Claude Code conversation. Ingests all ~/.claude/projects/*/*.jsonl into an archive DB every 15 minutes (launchd on macOS, Task Scheduler on Windows, cron / systemd on Linux, cron.d on containerized cloud agents — see references/cloud-deployment.md), so any new session can recall verbatim what you did before — across all projects, all sessions, all tool_use inputs and tool_result outputs. **Two interchangeable backends, same source, Cargo feature flag**: (1) DEFAULT `cargo build --release` → sqlite + sqlite-vec at `~/claude-archive/sessions.db` (single-device, sub-10ms local). (2) `cargo build --release --features pg-backend` (or `install.sh --with-pg` / `install-container.sh`) → PostgreSQL 17 + pgvector on a shared VPS, accessed via local `pgsearchd` daemon over a unix-socket r2d2 connection pool (cross-device, ~280-400ms warm via daemon, ~1s direct without). Cloud / container deploys skip pgsearchd and connect direct to a same-host PG container over Docker network — sub-10ms LAN. Same DB shared across Mac dev + cron agents = "global brain" cross-machine recall. See references/pg-backend.md and references/cloud-deployment.md. Two query modes: `csearch` (FTS5 lexical / PG GIN, exact phrase / boolean / prefix; default full-content per hit since v1.17) and `vsearch` (semantic via Ollama + bge-m3 — multilingual SOTA, concept queries, synonym / cross-language matching, strong Chinese). Activates when user wants to (a) install the archive on a new machine (macOS/Linux/Windows/**Docker container / cloud agent / VPS**), (b) query past sessions ("上週/昨天/之前做了什麼", "csearch ...", "vsearch ...", "查歷史對話 / past conversations / semantic search"), (c) install or troubleshoot Ollama / sqlite-vec / pgvector / pgsearchd daemon stack, (d) migrate from sqlite to PG backend (rebuild with --features pg-backend), (e) tune SQLite performance, (f) troubleshoot FTS5 syntax / ingest issues, or (g) **wire a cloud / cron / containerized Claude CLI to a shared archive_main PG (cross-machine recall)**.
version: 1.30.0
allowed-tools:
  - Read
  - Write
...
| Trigger | Action |
|---|---|
| User asks you to recall something from a past session ("上週那個 X 怎麼設的？", "we discussed Y last Thursday") | Run `osearch '<natural-language paraphrase>' [project]` first (default front door, pg-backend — fuses semantic + lexical + distilled via RRF); `vsearch` (pure semantic) / `csearch` (precise literal: IP / hostname / file path / FTS5 syntax) are manual shortcuts to skip the funnel. On sqlite-only backend osearch is not a subcommand at all — `vsearch` + `csearch` are the whole toolset, and the preflight rule does not apply there (v1.29) |
| User about to SSH / dig logs to identify someone or look up history (".98 是誰?", "上次怎麼處理 X 的") | **Discipline layer** (preflight hook) blocks the SSH/log-grep until `osearch` runs (since v1.28 — vsearch/csearch alone no longer unlock). The answer is often already in archive. See `references/discipline-layer.md` |
...
### Containerized / cloud-agent (Docker) — v1.18+

For a Claude CLI running unattended in a container (e.g. self-hosted VPS cron agent), Ollama in a sibling container, shared PG with other clients:

```bash
# Inside the container:
cd /root/.claude/skills/claude-session-archive-skill/scripts
./install-container.sh                       # build + symlink + hooks + cron.d + doctor
# Then customize the CLAUDE.md template:
cp ../templates/CLAUDE.md.cloud-agent.template /root/.claude/CLAUDE.md
# (edit the {{PLACEHOLDERS}})
```

This path skips the `pgsearchd` daemon (direct PG over Docker network is already sub-10ms LAN) and uses `cron.d` instead of launchd / systemd. Full walkthrough + gotchas: `references/cloud-deployment.md`. CLAUDE.md template covers agent identity / hard constraints / sub-agent rules / project-filter discipline.
...
**Only `osearch` opens the preflight sentinel (since v1.28).** CLAUDE.md preflight default is **osearch first**; `vsearch`/`csearch` are manual shortcuts for "skip the funnel, go straight to one leg" and — on their own — do **not** unlock the gate until osearch has run once this session (they only *refresh* an already-opened session).
...
> `vsearch '192.168.11.41 是哪台設備'` returns generic device-list hits — the embedding weights the meta words ("是哪台設備") about as much as the IP, diluting matches against the specific historical discussion of that IP. **Use csearch for literal IPs / hostnames / IDs**: `csearch '"192.168.11.41"' network` (note the phrase quotes — IPs contain `.` which is an FTS5 operator). Since csearch is `ORDER BY ts DESC`, the most recent definition of `.41` lands at top — exactly what you want for "what is this device?" lookups. Verified failure mode 2026-05-18: this misuse cost 5+ probe steps and produced the wrong identification.

vsearch sweet spot vs blind spot (verified 2026-05-04, refined 2026-05-18):
...
- ❌ **Weak on "latest state of X" queries when X is a known literal.** vsearch ordering is `ORDER BY embedding <=> v` — no temporal axis. Older entries with slightly tighter cosine can outrank newer entries with the actual current answer. For "what is .41 now?" / "current password for X?" / "latest config of Y?" the right tool is csearch (literal X + ts DESC built in).
...
|---|---|---|
| `archive-preflight.sh` (`.ps1` on Windows) | PreToolUse (Bash + Read) | **Reactive**. **Hard-denies** raw sqlite3 SEARCH against sessions.db (LIKE / MATCH / msg_fts / GLOB) regardless of sentinel — csearch is the only supported interface for content. Sentinel-gated: sqlite3 metadata queries, memory-file grep/Read, SSH/local log dig, `git log --grep`. **Only `osearch` opens a session** (sets `osearch_marker` + sentinel, since v1.28); `vsearch`/`csearch` refresh the sentinel only after osearch has opened it. Sentinel (`/tmp/claude-archive-preflight-<sid>`) is **TTL 30 min**, forcing a re-search after the post-compact gap |
| `auto-osearch-on-prompt.sh` | UserPromptSubmit | **Proactive**. On prompts containing identity / history / status / question keywords, auto-runs `crs osearch <prompt>`, injects top hits as `additionalContext`, and — after osearch actually runs — sets the `osearch_marker` + sentinel (a cooldown skip / timeout kill sets neither, so the gate stays closed) |
...
**6. Memory file is a stale index, not source of truth (since v1.9.0 — enforced by hook).** `~/.claude/projects/*/memory/*.md` is hand-curated — only what someone bothered to write down. For roster / device / credential / history queries, the canonical source is the archive (full transcripts). The `archive-preflight.sh` hook denies `grep`/`Read` on memory files until `osearch` runs once in the session (since v1.28 — `vsearch`/`csearch` alone no longer unlock), preventing the antipattern of grepping memory and silently missing data that's actually in the archive. `MEMORY.md` itself is exempted (it's auto-loaded by the system anyway). Once the sentinel is set, memory access is unblocked for the rest of the session — useful for "I just discussed this in this session" lookups.
...
**8. Sentinel has a 30-minute TTL (since v1.11.0).** The preflight sentinel expires 30 min after creation; opened by `osearch` and thereafter refreshed by any `osearch`/`vsearch`/`csearch` (since v1.28 only `osearch` can *open* a session — see rule 9). Motivation: post-compact, the model loses procedural memory of having searched but `session_id` (and thus the sentinel file) persists. Without TTL, an idle gap or compact would leave the sentinel valid forever, allowing the model to skip the archive in violation of the rule's intent. `auto-osearch-on-prompt.sh` refreshes the sentinel on every prompt that matches archive-intent keywords, so active conversations rarely hit the TTL.

**9. Only `osearch` opens a session (since v1.28.0 — enforced by hook).** The preflight hook sets a per-session `osearch_marker` (`/tmp/claude-archive-osearch-<sid>`) **only** on an `osearch` invocation; `vsearch`/`csearch` refresh the sentinel **only after** that marker exists. Before v1.28 all three unlocked equally, so CLAUDE.md's "first archive query MUST be osearch" was advisory-only and got silently bypassed (2026-07-20: opened with csearch/vsearch on a disk-full alert → only noise → SAN-vendor misID → near-miss on a false reading; osearch surfaced the answer immediately). osearch is the strict superset (RRF-fuses the vsearch + csearch legs + distilled orient), so making it the sole opener costs nothing and closes the drift.
...
>
> Hook (`~/.claude/hooks/archive-preflight.sh`) 在 sentinel valid 前會擋下：對 `~/claude-archive/sessions.db` 的 raw `sqlite3`（含 metadata）、對 `~/.claude/projects/*/memory/*.md` 的 grep/Read、`ssh ... grep ... /var/log/...` 遠端 log 翻找、本機 log grep、`git log --grep`。**只有跑過 `osearch` 才會開場解鎖 sentinel（v1.28 起）**；`vsearch` / `csearch` 單獨不解鎖，只在 osearch 開過場後才能 refresh。
>
...
- **Before SSH-ing into a device to grep logs / identify someone** → osearch first, the answer is often already there
...
## Auto-context bridging (crs gen-recent + auto-osearch-on-prompt)
...
- **UserPromptSubmit hook** (`auto-osearch-on-prompt.sh`) — on prompts with
  identity/history/status/question keywords, auto-runs `crs osearch <prompt>`
...
│   └── discipline-layer.md           # hooks: archive-preflight + auto-osearch-on-prompt
└── scripts/
    ...
    ├── auto-osearch-on-prompt.sh     # UserPromptSubmit hook — auto-osearch on identity/history/status prompts, injects top hits + sets osearch_marker + sentinel (after osearch actually runs)
...
- 2026-09-18 v1.30.0 **Skills distributed through PG + row importance in osearch.** Two problems with one root: knowledge that exists is not the same as knowledge that gets used. (a) `inap-release-manager` held a complete `instructions.md` + `references/` — including a CR rule learned by getting it wrong — but had no `SKILL.md`, and Claude Code only discovers skills through `SKILL.md`. It had therefore never been loaded once: directory present, content complete, nothing ever errored, and the same release work restarted from scratch each time. The retrieval was never the problem (`osearch 'iNAP Lab+ 變更報告'` returns the report as hit #1); the skill was simply invisible. (b) `osearch` had no notion that one row matters more than another — the only weight was recall×2 for natural-language queries, so a throwaway `ls` and a finished change report competed on similarity alone. Adds: **`skill-init` / `skill-publish` / `skill-sync` / `skill-get` / `skill-list`** — skills are authored in git and published into PG, split across two tables because their update rates differ ~280x (`skill_registry.payload` = JSON {path→text} of the small knowledge, 38 KB measured; `skill_asset` = one row per large/binary template, 15.8 MB for the same skill, fetched only when a document is actually produced and skipped by sha when unchanged — verified: a re-publish uploads 0 of 9 assets). `bytea`, never Large Object: `lo` carries its own ACL in `pg_largeobject_metadata` where RLS cannot see it, and the employee deployment is built entirely on RLS. `skill-sync` materialises into `~/.claude/skill-library/` — deliberately **not** `~/.claude/skills/`, so a synced skill does not silently rejoin the always-loaded description list (measured 14,528 chars for 35 skills) that this mechanism exists to relieve; it verifies sha256 and refuses `..`/absolute paths, and never executes anything. **osearch gains a 4th leg** over `skill_registry.description_embedding`, printing a `📌 現成 SOP` pointer above the rows, gated at distance 0.62 so unrelated queries stay silent (verified both directions). Routing was measured *before* building it: 11/12 realistic queries reach the right skill top-1 from description alone — the single miss was a skill whose entire description is the word "metadata", a content bug rather than a retrieval one. **`importance-backfill`** scores rows 0-4 by deterministic string rules (document written / completion reported / standing decision), filled on a later pass rather than at ingest because there are three separate `INSERT INTO msg` sites and one rule must not live in three places; `importance` is nullable so "not scored" stays distinguishable from "scored, ordinary". `osearch_ranked` multiplies the fused score by `1 + 0.15·min(importance,4)` — a **re-rank only**, applied to rows the three legs already returned, so it can lift a deliverable above surrounding chatter but can never inject a row nothing matched. First implementation updated row-at-a-time and measured 424s for 2000 rows against bluesea (0.21s each — the WAN round trip, not the work); rewritten as one multi-row `UPDATE … FROM (VALUES …)` per chunk → 6000 rows in 35.7s, the same lesson the batched ingest `INSERT` had already recorded a few hundred lines above. Also ships `scripts/skill-health-check.sh`, which scans `~/.claude/skills` for the (a) failure mode; its first version flagged 8 skills including several that were demonstrably working, because "no frontmatter" is not "broken" — Claude Code falls back to the first H1 — so severity is now three-tier (BAD = content but no `SKILL.md`, blocks; SAD = loads but with a weak title-derived trigger; INFO = empty dir / slash-command pack / cache) and the red path is verified by removing a `SKILL.md` and confirming exit 1.
- 2026-07-20 v1.28.0 **Preflight: only `osearch` opens a session.** Previously the preflight hook refreshed the sentinel for **any** of `osearch|vsearch|csearch`, so CLAUDE.md's "the first archive query MUST be osearch" had zero enforcement — the model could (and did, 2026-07-20 on a disk-full false alarm) open with `csearch`/`vsearch`, get only noise, misidentify a SAN vendor, and nearly act on a false reading; re-running the same query as `osearch` immediately surfaced the decisive context. Fix: `archive-preflight.sh`/`.ps1` now set a separate per-session `osearch_marker` (`/tmp/claude-archive-osearch-<sid>`) **only** on an `osearch` invocation; `vsearch`/`csearch` refresh the sentinel **only after** that marker exists (they extend an already-opened session, they cannot open one). All deny messages reworded "run OSEARCH first (default front door)". `auto-osearch-on-prompt.sh`/`.ps1`: the sentinel + marker are now set **after** osearch actually runs — moved out from before the cooldown/timeout early-exits, which had been unlocking the gate on the two paths where the archive was never actually consulted. Docs: `references/discipline-layer.md` rewritten for osearch-only unlock + the 2026-07-20 incident; SKILL.md behavior statements updated. No binary/schema change — hook + docs only. Also folds in accumulated hook drift between the repo and deployed copies (repo `archive-preflight.sh` predated even the osearch mention).

- 2026-05-21 v1.18.0 **Cloud / containerized agent deployment first-class support.** Adds three new files and one section without splitting into a separate skill (rejected — 75% of source / SQL / hook / CLI is shared between local and cloud; only install steps and CLAUDE.md content diverge): `references/cloud-deployment.md` covers when-to-use, architecture differences (Ollama via Docker DNS, cron.d vs launchd, no pgsearchd daemon needed on LAN), step-by-step manual install + the 8 gotchas that bit during the bluesea daily-ci-agent wiring (Text-file-busy on running-binary cp, `pkill -f` self-kill via own cmdline matching, defunct PID-1 zombie reap, vsearch leading-dash project filter eaten by clap, `crs doctor` hardcoded "localhost:11434" misleading display string, ssh-docker-exec quoting depth, container TZ vs cron schedule, multi-network DNS resolution). `scripts/install-container.sh` is idempotent (jq-merge into settings.json preserves any user-added hooks), skips pgsearchd intentionally, renders `/etc/cron.d/crs-build` from `templates/crs-build.cron` with env substitution, runs `crs doctor` for sanity, leaves CLAUDE.md alone (template is the seed). `templates/CLAUDE.md.cloud-agent.template` is a generalized version of what landed on bluesea daily-ci-agent (v3 after two rounds of iteration): agent identity / endpoints / preflight enforcement / FTS5 cheatsheet / vsearch-vs-csearch decision table / "DB is historical snapshot, NOT live state" warning / strict project filter / hard constraints / sub-agent (Task tool) discipline / report-with-evidence-links style / 4-phase SOP / over-engineering anti-patterns / high-risk escalation list. Built on the v1.17.1 OLLAMA_HOST env support — without that, containerized deploys could not work at all. SKILL.md description gains "containerized / cloud-agent" trigger keywords + a new "Containerized / cloud-agent (Docker)" install section. README.md gets a parallel section pointing at the same files. No binary, hook, or schema changes — purely additive deployment ergonomics. The decision NOT to spin off a `claude-session-cloud-archive-skill` was made via the project's own technical-decision SOP (first principle: query CLI / source / SQL / hook concept are 100% shared; what differs is install scripts + CLAUDE.md content; the over-engineering trap of "make a separate skill to keep things clean" was avoided by adding 3 files instead of 1 skill).

- 2026-05-21 v1.17.1 **Ollama URL env override (`OLLAMA_HOST` + `CRS_OLLAMA_URL`).** Bug: `const OLLAMA_URL = "http://localhost:11434/api/embed"` was hardcoded, breaking every deployment where Ollama isn't on localhost. Mac dev machines hide this — they all run Ollama locally — but the moment you put crs in a Docker container with Ollama in a sibling container, every `vsearch` / `crs build` embed call fails. Fix: replace the `const` with `fn ollama_url() -> String` honoring `CRS_OLLAMA_URL` (full URL incl. `/api/embed`) first, then `OLLAMA_HOST` (host root, append `/api/embed` — matches Ollama's own convention so any container with `OLLAMA_HOST` already set just works), then the localhost default. Found while wiring the bluesea daily-ci-agent (Oracle Cloud aarch64 Rocky 9 container) to the shared archive_main PG: `OLLAMA_HOST=http://ollama:11434` was already in the container env but crs ignored it. After patch, container-internal `vsearch` resolves embeds via Docker DNS to the `ollama` sibling container correctly. Note `crs doctor` still prints "Ollama reachable at localhost:11434" — that's a hardcoded display label, not a measurement of the configured endpoint; v1.18+ docs call this out as a known cosmetic.
...
- 2026-05-06 v1.10.0 **Discipline layer expansion — log-dig blocking + UserPromptSubmit auto-vsearch + dedicated reference doc.** Builds on v1.9.0's preflight hook with three additions:
  ...
  - `install.sh` extended: step 6 hook copy now handles `auto-osearch-on-prompt.sh` with detection-and-skip for customized versions; step 8c registers the UserPromptSubmit hook in `~/.claude/settings.json` idempotently. Existing installs: re-run `install.sh` — preflight gets new rules 5+6, UserPromptSubmit hook installed and registered.
  - **Windows port pending** — `archive-preflight.ps1` keeps rules 1-4 (from v1.9.0); rules 5+6 PowerShell port and `auto-osearch-on-prompt.ps1` are TODO. Windows installs currently get partial discipline (preflight v1.9.0 behavior).
