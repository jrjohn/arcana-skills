...
**1. Any service can contribute to the archive without going through `crs build`.** The canonical ingest path is `crs build` scanning `~/.claude/projects/*/*.jsonl`, but the `msg` table is just `(session_id, project, seq, ts, role, tool_name, content)` with PK `(session_id, seq)`, a GENERATED `content_tsv`, and a nullable `embedding`. So **any** producer of a verbatim conversation can become csearch/vsearch-able by doing a plain `INSERT … ON CONFLICT (session_id, seq) DO NOTHING` with `embedding` left **NULL** — the existing `crs embed-missing` pass (it embeds *every* `embedding IS NULL AND length(content) >= 5` row globally, no source filter) fills the vector on its next cycle. csearch works immediately (content_tsv is GENERATED on insert); vsearch lights up after the next embed cycle. Proven by ingesting headless Claude `--output-format stream-json` agent transcripts (a BPMN AI-task worker reads the per-node stream-json log and inserts assistant text + thinking + tool_use, user tool_results, and the result's structured_output verbatim, `project='aaf'`, `session_id='<instance>__<node>'`). The agent runs are now recalled by csearch/vsearch exactly like Mac sessions — the verbatim transcript, not a condensed summary, is the source of truth. Takeaway for new producers: don't build a parallel store or a "refined" copy; INSERT verbatim into `msg`, leave embedding NULL, let `crs embed-missing` do the vectors.
