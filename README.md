...
| `arch-qube-skill` | Usage guide for **Architecture Qube** (`arch-qube`) — the AI-powered Architecture Quality Gate run as the fleet's "Architecture Qube" Jenkins stage. Covers running scans, the 21 rules / 14 framework profiles, fixing MVVM / Clean Architecture / layer-direction violations, two-stage AST+AI analysis, scoring, CI integration (Jenkins/SonarQube/GitHub Actions/pre-commit), and adding custom rules/profiles as YAML (no code change) |
