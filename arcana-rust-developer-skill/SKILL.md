...
2. **Duplicate its ENTIRE file set** (domain model + DTO + mapper, SQL migration, repository trait + impl, service trait + impl, Axum handler + route registration, gRPC proto + Tonic service impl, AppState/DI wiring, mock data, and unit tests per layer) 1:1, keeping every layer.
