...
| `merge-flow` | Start→Merge(ai)→Release(ai)→End | Merge: agent re-checks `gh pr view/checks` (open + green + no conflicts) then `gh pr merge --squash --delete-branch`. Release: via agent `/task/release` — FIRST a scoped claude **readme-sync** pass (compares README version claims against the repo's dependency manifests via `gh api`, commits `docs: sync README versions with manifests` if stale), THEN deterministic `npx release-please@16 github-release` + `release-pr` (released detection = ground-truth latest-tag before/after, not output parsing); skips repos without release-please-config. `POST /task/readmesync {repo}` also works standalone. |
