...
### IR-3.5 表單驗證必有「就地說明 + 修正路徑」
- 被拒的輸入**必須**在欄位下方顯示文字說明:**規則是什麼、怎麼修**(例:「需小寫字母開頭,僅 a-z 0-9 -」),能自動修正的**附一鍵修正**(「改用 leave-request-form」)。
- **禁**:只有紅框/紅圈、或只把送出鈕 disable 而無說明 —— 使用者會卡死不知道錯在哪(死路 UX)。
- 驗證訊息與規則要一致:placeholder 示範 + hint + 後端 guard 三者同一條規則。
...
- [ ] **IR-3.5**:每個會被拒的輸入都有就地文字說明+修正建議?沒有「紅框無說明」死路?
