...
## 1.5 鐵律（Iron Rules — 可檢驗、gate 強制、慣性失誤防線)★

> 蒸餾自 `wholiver/swiftui-design-skill` 的 **anti-AI-slop 6 規則** + Refactoring UI / NN-g / data-table 最佳實踐(Pencil&Paper、LogRocket),轉成 **Angular/web 可檢驗條文**。這四條是「一再復發、gate 該擋、PM 該 NOGO」的具體失誤 —— 每條都寫成能被自動量或人一眼判的形式。

### IR-1 嵌入式編輯器 = 吃滿寬 **且** 吃滿高;側板未用時 collapse 到 0
- bpmn-js / form-js / code / chart 等嵌入編輯器**必須 100% 寬 + 填滿可用高**(`flex:1; min-height:0`,或 `height:100%` 於一個有界高的父容器)。
- **禁**用寫死的 `height:520px` / `max-height:62vh` 當主編輯器高 → 那會「框只有半高、下面一片空」(組織圖 tab 的成因)。
- **情境側板未使用時必須用 `@if`/`*ngIf` 移除(collapse 到 0 寬),不是留一個空欄**。留空欄 → 編輯器被吃掉 ~1/3 寬(設計師畫布「只有 2/3」的成因)。選取節點才展開側板(對應 §2 案例 2 的正解)。

### IR-2 多列資料 = table + 搜尋 + 分頁 + 排序(禁卡片堆 / 樹複製)
- 任何「> ~8 列或無上限」的資料清單**必須**:`<table>`(sticky header)+ **搜尋框** + **分頁**(或虛擬捲動)+ **可排序欄**;空 / 無結果 / 載入 三狀態齊。
- **欄寬混合策略**:可預測欄(狀態/日期/動作)固定寬;文字欄 `flex`;流動欄給 `min-width` 免塌成細條。
- **禁**:把多列資料做成卡片橫向堆疊,或把「清單/人員/列表」tab 做成「樹的複製」。**「清單/人員/列表」語意 = table,不是卡片牆。**

### IR-3 內容區 = 填滿視窗高 **或** 縮到內容,不准中間態
- 主內容卡片:要嘛 `flex:1` 填滿剩餘視窗高(內部捲動),要嘛縮到內容高。**禁**寫死半高/固定 vh 造成「上面有東西、下面一大片空」。
- flex/grid 欄等高造成的尾部空白(sibling 較高)屬正常,不算違規。

### IR-4 禁 AI-slop 樣式（蒸餾自 anti-slop 6 規則)
- **禁**:紫藍漸層當主色、emoji 當功能圖示、「圓角卡+左粗邊」到處貼、把每個像素都填滿。
- **每個元素都要有存在理由**(system-first, don't fill);寧留乾淨留白,不塞裝飾。單一暖色 accent + 一致 icon set(bootstrap-icons)+ 走 token,不自創第二套色。

**5 維自評(每項 ≥7/10 才算過,蒸餾自 anti-slop skill)**:哲學一致 / 視覺層次 / 細節執行 / 功能完整 / 原創(不像 AI 模板)。
...
| **IR-1 編輯器填充** | 嵌入編輯器(bpmn/form/canvas)高度 ≥ 可用內容區的 ~80%,且寬度 ≥ ~90% | WARN |
| **IR-1 空側欄** | 主編輯器旁沒有「寬 > 200px 但內部無內容」的常駐欄(未選取即應 collapse) | WARN |
| **IR-2 清單即 table**（AI gate 語意判) | 多列資料(>8 列)用 `<table>` + 搜尋 + 分頁,非卡片堆/樹複製 | WARN→FAIL |
...
- [ ] **IR-1**:嵌入編輯器**吃滿寬且吃滿高**?情境側板**未選取時 collapse 到 0**(非留空欄)?
- [ ] **IR-2**:多列資料是 **table + 搜尋 + 分頁 + 排序**?不是卡片堆/樹複製?三狀態齊?
- [ ] **IR-3**:內容區**填滿視窗高或縮到內容**,沒有寫死半高造成的大片空白?
- [ ] **IR-4**:無 AI-slop(紫藍漸層/emoji 功能圖示/裝飾性圓角卡左邊框)?每元素有存在理由?
- [ ] 次要/進階/稀用內容都進了 toolbar 或漸進揭露容器,沒各佔一欄?
- [ ] 常駐/情境側板 ≤ 1?更多用 drawer?
- [ ] 有明確主導區(視覺層次 3–4 層),不是等權重平分?
- [ ] 全走 token,無裸 hex/px?
- [ ] 空/載入/錯誤狀態齊(見 rubric §7)?
- [ ] **5 維自評每項 ≥7/10**(哲學一致/視覺層次/細節/功能/原創)?
