...
## ⚠️ 重要提醒：必須使用 Skill 內建模板

**生成 UI Flow 時，必須使用 `templates/screen-types/` 內的模板作為起點。**
**生成 主要 index.html 時，必須使用 `templates/ui-flow/index.html` 模板作為起點。**
**生成 主要 ui-flow-diagram.html 時，必須使用 `templates/ui-flow/ui-flow-diagram.html` 模板作為起點。**

### 強制步驟

1. **複製畫面模板**
   ```bash
   # 根據畫面類型，從 screen-types/ 複製對應模板
   cp ~/.claude/skills/app-uiux-designer.skill/templates/screen-types/auth/login.html ./04-ui-flow/auth/SCR-AUTH-001-login.html
   ```

2. **替換模板變數**
   - `{{PROJECT_ID}}` → 專案代碼
   - `{{SCREEN_ID}}` → 畫面 ID (如 SCR-AUTH-001)
   - `{{TARGET_SCREEN}}` → 導航目標 (來自 SDD Button Navigation)

3. **客製化樣式**
   - 修改 `shared/{{project}}-theme.css`
   - 保留模板的結構和導航邏輯
...
| 類型 | 模板路徑 |
|------|---------|
| 登入 | `screen-types/auth/login.html`, `login-ipad.html`, `login-iphone.html` |
| 註冊 | `screen-types/auth/register.html` |
| 忘記密碼 | `screen-types/auth/forgot-password.html` |
| 角色選擇 | `screen-types/auth/role-selection.html` |
| Dashboard | `screen-types/common/dashboard.html` |
| 列表頁 | `screen-types/common/list-page.html` |
| 詳情頁 | `screen-types/common/detail-page.html` |
| 表單頁 | `screen-types/common/form-page.html` |
| 設定頁 | `screen-types/common/settings.html`, `settings-iphone.html` |
| 個人檔案 | `screen-types/common/profile.html` |
| 搜尋頁 | `screen-types/common/search.html` |
| Onboarding | `screen-types/onboarding/onboarding.html` |
| 空狀態 | `screen-types/states/empty-state.html` |
| 載入中 | `screen-types/states/loading-state.html` |
| 錯誤狀態 | `screen-types/states/error-state.html` |
| 成功狀態 | `screen-types/states/success-state.html` |
| Tab Bar | `screen-types/components/tab-bar.html` |
| 導航列 | `screen-types/components/navigation-bar.html` |
| Modal | `screen-types/components/modal.html` |

### 禁止事項

- ❌ **禁止從零開始手寫 HTML 畫面**
- ❌ **禁止跳過模板直接產出**
- ❌ **禁止忽略 Button Navigation 定義**
...
| `{{PROJECT_DESCRIPTION}}` | 專案簡述 | `國小英語學習 App` |
...
| `{{ONBOARD_COUNT}}` | ONBOARD 模組畫面數 | `3` |
| `{{DASH_COUNT}}` | DASH 模組畫面數 | `2` |
| `{{FEATURE_COUNT}}` | FEATURE 模組畫面數 | `10` |
| `{{REPORT_COUNT}}` | REPORT 模組畫面數 | `4` |
| `{{SETTING_COUNT}}` | SETTING 模組畫面數 | `12` |
