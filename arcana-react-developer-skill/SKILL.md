---
name: arcana-react-developer-skill
...
4. **Add features by copying the nearest working feature, then adapting** (see [🔁 Adding a feature](#-adding-a-feature--copy-the-nearest-working-feature-not-from-scratch)) — the [File-by-File Feature Recipe](#file-by-file-feature-recipe) below is the completeness checklist.
...
2. **Duplicate its ENTIRE file set** (model, validator, service interface + impl, repository interface + impl, DTO, mapper, mock repository, provider binding + `useXxxRepository` hook, ViewModel hook, component, NavGraph methods, route, tests) 1:1, keeping every layer.
