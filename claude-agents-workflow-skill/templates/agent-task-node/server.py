...
def prompt_root_cause(p):
    ...
    """
    # 流程變數一律走 _pv:它同時找頂層與 `data`,而 worker 送來的形狀兩種都出現過。
    # 這段是 2026-08-31 寫的,那時還沒有這條規則;pv_discipline_selftest 會擋。
    pr = p.get("prUrl") or ""
    rep = _pv(p, "testReport")
    if not isinstance(rep, str):
        rep = json.dumps(rep, ensure_ascii=False) if rep else ""
    n = _pv(p, "testAttempts")
    return (
...
def _write_workspace_claude_md(root, payload):
    ...
    body = (
        ...
        ">\n"
        "> 違反這條的代價:**沒有 hook 會擋你,所以代價落在後面,而且由整條流水線付** ——\n"
        "> 你會重新提出上一輪已經被否決的設計,而下游看不出那是重做,於是它再被否決一次。\n\n"
        "```bash\n"
        "# 語意查:忘了當時用什麼字\n"
        "%s \\\n  %s vsearch '<白話問題>'\n\n"
        ...
        "## 違反這條的代價\n\n"
        "**沒有 hook 擋你,這條靠自律** —— 所以代價落在後面,而且是整條流水線在付:\n"
        "你會重新發現已經被發現過的事、重新提出上一輪已經被否決的設計,而**下游看不出那是重做**。\n"
        "被否決過的東西再送一次會再被否決一次,那一輪的成本沒有人回收得了。\n"
...
def gemini_security_gate(pr_url):
    ...
    try:
        cs = _gh_json(["api", "--paginate", "repos/%s/%s/issues/%s/comments" % (owner, repo, num)])
        comments = [{"login": (c.get("user") or {}).get("login"), "body": c.get("body"),
                     "updated_at": c.get("updated_at")} for c in cs]
        pr = _gh_json(["api", "repos/%s/%s/pulls/%s" % (owner, repo, num)])
        labels = {l.get("name") for l in pr.get("labels", [])}
        commits = _gh_json(["api", "--paginate", "repos/%s/%s/pulls/%s/commits" % (owner, repo, num)])
...
def block_for_security(pr_url, reason):
    ...
    try:
        pr = _gh_json(["pr", "view", pr_url, "--json", "labels"])
